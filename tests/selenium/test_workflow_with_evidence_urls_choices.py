...
class JudgmentRegisterTest(SeleniumBaseRemoteTest):
    ...
    def test_answer(self):
        ...
        answer = LabelingAnswer.objects.get(rater=rater, item=item, workflow=workflow)
        self.assertEqual(LabelingAnswer.objects.all().count(), 2)
...
class JudgmentWithoutJudgmentTest(SeleniumBaseRemoteTest):
    def test_answer(self):
        ...
        with self.assertRaises(ObjectDoesNotExist):
            LabelingAnswer.objects.get(rater=rater, item=item, workflow=workflow)
        self.assertEqual(LabelingAnswer.objects.all().count(), 1)
